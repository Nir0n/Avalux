//
//  Events.swift
//  MySampleApp
//
//
// Copyright 2017 Amazon.com, Inc. or its affiliates (Amazon). All Rights Reserved.
//
// Code generated by AWS Mobile Hub. Amazon gives unlimited permission to 
// copy, distribute and modify it.
//
// Source code generated from template: aws-my-sample-app-ios-swift v0.10
//

import Foundation
import UIKit
import AWSDynamoDB

class Events: AWSDynamoDBObjectModel, AWSDynamoDBModeling {
    
    var _userId: String?
    var _dateOfEvent: Set<NSNumber>?
    var _placeOfEvent: [String: String]?
    var _typeOfEvent: String?
    
    class func dynamoDBTableName() -> String {

        return "avalux-mobilehub-2079428285-Events"
    }
    
    class func hashKeyAttribute() -> String {

        return "_userId"
    }
    
    override class func jsonKeyPathsByPropertyKey() -> [AnyHashable: Any] {
        return [
               "_userId" : "userId",
               "_dateOfEvent" : "dateOfEvent",
               "_placeOfEvent" : "placeOfEvent",
               "_typeOfEvent" : "typeOfEvent",
        ]
    }
}
